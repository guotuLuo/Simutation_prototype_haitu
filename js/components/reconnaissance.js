...
class Reconnaissance {
    ...
    createMarker() {
        ...
        this.marker.bindPopup("侦察样机: " + this.id).openPopup();
        // 绑定右键菜单显示事件

        const latLng = this.marker.getLatLng();
        this.marker.bindPopup("侦察样机: " + this.id + "<br>经纬度: " + latLng.lat.toFixed(6) + ", " + latLng.lng.toFixed(6)).openPopup();

        // 监听拖动过程中的事件
        this.marker.on('drag', (event) => {
            const latLng = event.target.getLatLng();  // 获取当前经纬度
            this.marker.setPopupContent("侦察样机: " + this.id + "<br>经纬度: " + latLng.lat.toFixed(6) + ", " + latLng.lng.toFixed(6));
    ...
    setSpeed() {
        ...
        this.marker.getPopup().setContent("侦察样机: " + this.speed);
        axios.post("http://127.0.0.1:8081/api/reconnaissances/setSpeed", {
            id: this.id,
            speed: this.speed
        }).then(response => {
            console.log(`侦察样机 ${this.id} 速度设置成功`);
        }).catch(error => {
            console.error(`侦察样机 ${this.id} 速度设置失败:`, error);
        });
    }
    ...
    deleteRoute() {
        ...
        this.routeMarkers.forEach(marker => marker.remove());
        this.routeMarkers = [];
        const url = `http://127.0.0.1:8081/api/reconnaissances/deleteTrackPoint?uuid=${encodeURIComponent(this.id)}`;
        navigator.sendBeacon(url);
    }
